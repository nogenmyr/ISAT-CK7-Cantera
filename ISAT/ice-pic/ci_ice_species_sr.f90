...
subroutine ci_ice_species_sr
  ! Species selection based on species reconstruction error
  ...
  real(kind(1.d0)) :: FC(nr_max, 2*nsp1), z_ref
  real(kind(1.d0)) :: hin, hsin, z_FC(ns)
  ...
  write(0,*) 'Computing species order based on species reconstruction error.'
  ...
  open( lu_ce, file='ice_sp_order_sr_50.op' )
  open( lu_pl, file='ice_uusp_errors_sr_50.op' )
  ...
  z_ref = 0.d0
  do m = 1, nr_max
     z_ref = z_ref + dot_product(FC(m,1:ns), FC(m,1:ns))
  enddo
  z_ref = sqrt(z_ref/nr_max)
  ...
  num_rs = 1
  rep(1:num_rs) = (/3/)
  !rep(1:num_rs) = (/7,3,11,27,4,9,5,1,2,10,12,22,13,26,21,8,29,14,20,18,17,19/) !check chem.out
  !num_rs = 0
  !rep = 0
  ...
  species: do
     ...
     do s = 1, nsu
        ...
        do m = 1, nr_max
           ...
           ! Compute Errors
           err_z(s) = err_z(s) + dot_product(z_ICE - z_FC, z_ICE - z_FC)
        enddo
     enddo
     ...
     do i = 1, nsu
        ...
        err2_z(i)  = sqrt(err_z(i)/nr_max)/z_ref
...
end subroutine ci_ice_species_sr
