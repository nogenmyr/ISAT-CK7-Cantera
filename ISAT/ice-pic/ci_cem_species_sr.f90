...
subroutine ci_cem_species_sr
  ...
  integer            :: lu_in, lu_ce, lu_pl, lu_ck, lu_ct
  ...
  open( lu_ce, file='cem_sp_order_sr_2500.op' )
  open( lu_pl, file='cem_uusp_errors_sr_2500.op' )
  open( lu_ct, file='cem_sp_cputime_sr_2500.op')
  ...
  species: do
     ...
     do s = 1, nsu
        ...
        do m = 1, nr_max
           ...
           ! Compute Errors
           err_z(s) = err_z(s) + dot_product(z_CE - z_FC, z_CE - z_FC)
        enddo
     enddo
...
end subroutine ci_cem_species_sr
