...
subroutine ci_ice_facet_newton_lmap( kfa, r, h, p, tau0, r0, &  
                                r_normal0, r_tol, r_acc, tau, zg, Tg, zR, &
                                TR, A_g, niters, dr_norm, resid,  iter_flag )
...
! diagnostic flags:
!              = -10, linrmap converged, but not really
!              = -11, reject step: attenuate previous step and continue
!              = -12, end of do CEQ_TRY
!              = -13, end of do NEWTON
!              = -14, T out of bound, reset linrmap
  ...
  integer,    intent(out) :: niters, iter_flag
  ...
  integer   :: iter, info, iflag, indic_zpos(ns), kfa_g, &
       iflag_CEQ, iflag_ODE, iflag_lmap, indic_rpos(nrc)
  ...
  real(k_dp):: dr_norm_last, rg(nrc), dr(nrc), t_scale, &
       ...
       tau_inc, tau_lim, drga(nrc), A_tmp(ns+1, ns+1)

  ! lmap variables
  real(k_dp):: zcur(ns), dz(ns), taucur, dt, Tcur ! current values
  logical   :: RESET = .false. ! to reset zcur and taucur
  integer   :: retrieves

  !stats
  call routine_start(i_ci_ice_facet_newton_lmap)
  ...
  ! initialization     
  mn_tol    =  mn_fac * r_tol
  iter_flag = -10                 !  anticipate failure
  iflag_ODE =  1
  atten     =  1.d0
  ...
  NEWTON : do iter = 1, iter_max  !  start of Newton iterations =============
     ...
     call ci_linrmap( zcur, taucur, dz, dt, Tcur, h, p, zR, TR, A_g, iflag_lmap )

     if(iflag_lmap < 0) then
        ! linrmap failed due to ODE failure
     ...
     ! Count no. of ODE integrations
     if (iflag_lmap == 2 .or. iflag_lmap == 3) then 
        ! 2 - initialization; 3 - internal reset
        niters   = niters + 1
     else
        ! Count no. of retrieves
        retrieves = retrieves + 1
     end if
     ...
     if( dr_norm <= r_tol ) then
        if( maxval(abs(dz)) > 0.d0 .or. dt /= 0.d0) then
           ! do a full integration with map = 0
           call ci_chem_map0
           if(iter_flag == -5) exit NEWTON

           if( dr_norm <= r_tol ) then
              iter_flag = 0   ! success: Newton has converged
              exit NEWTON
           else
              ! reset zcur and taucur and continue
              call diagnostic_op( -10 )
              RESET = .true.
           endif
        else
           ! accept linrmap solution
           iter_flag = 0   ! success: Newton has converged
           exit NEWTON
        endif
     ...
     if( abs(beta)  > beta_lim ) then
        ! write(0,*)'ci_ice_pic_Facet_lmap: beta= ', beta !XXX
        ! re-solve exactly
     ...
     if ( iflag_CEQ < 0 )  then 
        if( iflag_CEQ <= -18 ) then  !  T out of range
           if( maxval(abs(dz)) > 0.d0 .or. dt /= 0.d0) then
              ! failure may be due to linrmap, so
              ! reset zcur and taucur and try linrmap again
              call diagnostic_op( -14 )
              RESET = .true.
              cycle NEWTON
           else
              dice(16) = dice(16) + 1.d0
...
contains  !-------------------------------------------------
  ...
  subroutine ci_chem_map0

    ! do a full integration with map = 0
    call ci_ice_chem_map( 0, tau, p, zg, h, TR, zR, A_tmp, iflag_ODE ) 
    ...
    ! recompute residual to check convergence
    dr      = r - matmul( BBT, zR )
    dr_norm = norm( dr )

  end subroutine ci_chem_map0
...
end subroutine ci_ice_facet_newton_lmap
