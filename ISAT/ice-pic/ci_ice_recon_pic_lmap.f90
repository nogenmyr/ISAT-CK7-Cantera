...
subroutine ci_ice_recon_pic_lmap( r, h, p, r_tol, r_acc, Sg_min, npt, np_acc, npmx,  &
...
real(k_dp), parameter :: tau_fac  = 3.d0   !  factor by which tau is allowed to change (>1)
...
integer, parameter :: calls_stop = 1e7  !  call on which to o/p statistics and stop
...
integer    :: ipt, ipl, niters, kfa, isub, icount(11), &
              ...
              rankA, rankB, npim, i, info, iflag_ODE
real(k_dp) :: zdum(ns), Tdum, stats(20), z(ns+1), dzdt(ns+1), dens, Sg(ns), &
              ...
              dzR_ex_best, zR_ch(ns), TR_ch, res_ch
...
elseif( nint(calls) >= calls_stop ) then !  output statistics and stop
   ...
   write(0,'(a,i5)')'total ODE integs  = ', icount(6) + icount(7)
   write(0,*)' '
   write(0,'(a,1p,e13.2)')'facet attempts    = ', icount(8)/float(icount(1))
   ...
   write(0,'(a,i5)')'total ODE integs  = ', icount(10) + icount(11)
   write(0,*)' '
   write(0,'(a,i5)')'ODE integrals     = ', icount(6) + icount(7) + icount(10) + icount(11)
   write(0,'(a,1p,e13.2)')'CPU time (secs)   = ', cpu_end - cpu_start
   write(0,'(a,1p,e13.2)')'CPU ( mu sec/call)= ', (cpu_end - cpu_start)*1.d6/calls
   write(0,*)'ci_ice_pic_lmap: stopping, calls = ', nint(calls)
...
if ( info < 0 )  then 
   write(0,*)'ci_ice_pic_lmap: initial CE failure, info = ', info
   pic_flag = -1
...
!  boundary point successfully identified    
 ...
 !  attempt to determine PI point 
      ...
      if( kfa == 0 ) then  ! interior      
         call ci_ice_pic_newton_lmap( r, h, p, r0, tau0, r_tol_n, r_acc_int, &
         ...
         if( diagnostic ) then 
            write(30,'(a,i3,i4, 4i3,1p,20e10.2)') 'PN (lmap): ', nint(calls), ipt, isub, &
         ...
         if( info >= 0 ) then
            ...
            call routine_failed(i_ci_ice_pic_newton_lmap)
         endif
                              
      else                ! on  facet                        

         call ci_ice_facet_newton_lmap( kfa, r, h, p, tau0, r0, r_normal, r_tol_f, r_acc_f,  &
                                   taug(ipt), zg(:,ipt), Tg(ipt), &
                                   zR(:,ipt), TR(ipt), A_ODE, niters, &
                                   dr_norm, resid, info )
         if( diagnostic ) then 
            write(20,'(a,i3,i4, 4i3,1p,20e10.2)') 'FN (lmap): ', nint(calls), ipt, isub, &
                 kfa, info, niters, dr_norm, s, sb, Tg(ipt), taug(ipt), sv_rat 
         endif
         ...
         if( info >= 0 ) then
            ...
            call routine_failed(i_ci_ice_facet_newton_lmap)
         endif
                                   
      endif
      
      if( diagnostic ) then 
         write(0,'(a,i3,i4, 4i3,1p,10e10.2)') 'ICE_PIC_lmap: ', nint(calls), ipt, isub, &
      ...
      if( info >= 0 ) then  !  PI point successfully generated
         ...
         if( step_size < step_min ) then ! step size too small
            if( diagnostic ) write(0,*)'ci_ice_pic_lmap: substep size collapsed, = ', step_size
...
end subroutine ci_ice_recon_pic_lmap
