...
subroutine ci_ice_pic_newton_lmap( r, h, p, r0, tau0, r_tol, r_acc, &
  ...
  real(k_dp), parameter :: atten_min = 0.1d0  ! min. attenuation of Newton step
  ...
  integer   :: i, iter, iflag_CEQ, iflag_ODE, iflag_CEM, iflag_lmap, k_min, rankA, rankB, npim
  ...
  real(k_dp):: dr(nrc), t_scale, mn_tol, alpha, r_ref, &
       ...
       SR(ns), SRA(ns), SRA_err, S_rat, A_tmp(ns+1, ns+1)
  ...
  !stats
  call routine_start(i_ci_ice_pic_newton_lmap)
  ...
  ! determine CE point, zg to set zcur
  ...
  if ( iflag_CEQ < 0 )  then 
     if( iflag_CEQ <= -18 ) then  !  T out of range
        ...
        info = -4
        call diagnostic_op( -4 )
        !stats
        call routine_stop(i_ci_ice_pic_newton_lmap)
        return
     else
        info = -4
        return
  ...
  do iter = 1, iter_max  !  start of Newton iterations =============
     ...
     !  Use linrmap to get zR and A_ODE
     if( RESET ) then
        zcur = zg
        taucur = tau
        Tcur = Tg
     ...
     call ci_linrmap( zcur, taucur, dz, dt, Tcur, h, p, zR, TR, A_ODE, iflag_lmap )
     ...
     !  Count no. of ODE integrations, 2 - initialization; 3 - internal reset
     if (iflag_lmap == 2 .or. iflag_lmap == 3) niters   = niters + 1
     ...
     if( dr_norm <= r_tol .and. iter > iter_min ) then
        if( maxval(abs(dz)) > 0.d0 .or. dt /= 0.d0) then
           ...
           if( dr_norm <= r_tol ) then
              ...
              ! not converged, reset and continue
     ...
     elseif( iter == iter_max ) then
        ...
        if( dr_norm <= r_acc ) then
           ...
           info = -3 !  failure: not converged in iter_max iterations
        endif
        exit
...
end subroutine ci_ice_pic_newton_lmap
